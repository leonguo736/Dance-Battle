...
/* The globals below are written by interrupt service routines, so we have to
 * declare them as volatile to avoid the compiler caching their values in
 * registers */
...
int main(void) {
    ...
    /* Note: we are passsing a pointer to up_dev to each ISR (using the HAL
     * context argument) as
    ...
    while (1)
        ; // main program simply idles, further program actions are handled by
          // interrupts
    return 0;
}
