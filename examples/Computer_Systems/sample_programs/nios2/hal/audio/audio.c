 ...
 *	1.	records audio for about 10 seconds when KEY[0] is pressed. LEDG[0] is
 *		lit while recording
 * 	2.	plays the recorded audio when KEY[1] is pressed. LEDG[1] is lit while
 *		playing
 ******************************************************************************/
int main(void) {
    ...
    while (1) {
        ...
        if (record) {
            ...
            if (buffer_index < BUF_SIZE) {
                num_read = alt_up_audio_record_r(
                    audio_dev, &(r_buf[buffer_index]), BUF_SIZE - buffer_index);
            ...
            if (buffer_index < BUF_SIZE) {
                num_written = alt_up_audio_play_r(
                    audio_dev, &(r_buf[buffer_index]), BUF_SIZE - buffer_index);
                /* assume that we can write the same # words to the left and
                 * right */
...
void check_KEYs(int * KEY0, int * KEY1, int * counter,
    ...
    KEY_value = IORD_ALTERA_AVALON_PIO_DATA(
        PUSHBUTTONS_BASE); // read the pushbutton KEY values
    while (IORD_ALTERA_AVALON_PIO_DATA(PUSHBUTTONS_BASE))
        ; // wait for pushbutton KEY release

    if (KEY_value == 0x1) // check KEY[0]
    {
        ...
        *KEY0 = 1;
    } else if (KEY_value == 0x2) // check KEY[1]
    {
