...
void audio_init_reg(void* virtual_base) {
  FIFO_REG =
      (uint32_t*)(virtual_base + ((unsigned long)(ALT_LWFPGASLVS_OFST +
                                                  AUDIO_SUBSYSTEM_AUDIO_BASE +
                                                  FIFO_REG_OFFSET) &
  ...
  if (audio_FD > 0) {
    ...
    if (bytes_read != AUDIO_BYTES) {
      fprintf(stderr, "Error: %d bytes expected from %s, but %d bytes read\n",
              AUDIO_BYTES, audio_CHAR_DEV, bytes_read);
      return 0;
    }
 ...
 * Parameter data: the sampling rate in samples per second (8000, 32000, or
 * 48000) Return: void
 */
