...
struct LobbyScreenState lobbyScreenStates[2] = {{0, 0, 0, 0}, {0, 0, 0, 0}};
...
struct GameScreenState gameScreenStates[2] = {{0, -1, -1, {0}},
                                              {0, -1, -1, {0}}};
...
void initGraphics(int s) {
  ...
  for (int i = 0; i < 2; i++) {  // Put the same thing on both buffers
    if (s == 0) {                // Init
      ...
      video_line(BORDER, HEIGHT - 50, WIDTH - BORDER, HEIGHT - 50,
                 COLOR_GAME_HLINE);
      video_line(50, BORDER, 50, HEIGHT - BORDER, COLOR_GAME_VLINE);
      video_line(WIDTH - BORDER, BORDER, WIDTH - BORDER, HEIGHT - BORDER,
                 COLOR_GAME_VLINE);
...
void updateGraphics(void) {
  ...
  if (screen == 0) {  // Init
    ...
    if (!titleSlideDone) {
      ...
      prevState->danceX =
          lerp(prevState->danceX, INIT_DANCE_X, INIT_TITLE_LERP);
      prevState->battleX =
          lerp(prevState->battleX, INIT_BATTLE_X, INIT_TITLE_LERP);
      ...
      prevState->statusY =
          lerp(prevState->statusY, INIT_STATUS_Y, INIT_TITLE_LERP);
...
int main(int argc, char** argv) {
  ...
  if (!esp_init(argc, argv)) {
    ...
    printf("Timespec Time taken: %f ms\n",
